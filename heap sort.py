#class code copied from heap tree
...
class HeapArray():
    ...
    def insert(self, e):
        ...
        while i!=1 and e < self.data[i//2]: #line modified
    ...
    def deletemin(self):
        ...
        y = self.data[self.currentsize] # assigning last element to root
        self.currentsize-1
        ...
        while ci <= self.currentsize:
            if ci < self.currentsize and self.data[ci] > self.data[ci+1]: #line modified
                #checking the min child for swapping
                ci += 1
            if y <= self.data[ci]:
                break
